...
[ ] W1-A  Add QuantumSitingResult dataclass to solvers/results.py
          Fields: backend, second_stage, n_candidates, quantum_candidates,
                  evaluated, best, runtime_quantum, runtime_classical

[ ] W1-B  Add qiskit, dimod, dwave-samplers to main requirements.txt
          Copy versions from Test_Quantum_examples/requirements.txt

[ ] W1-C  Create solvers/quantum_siting.py with empty stubs for all functions
...
[ ] W2-A  Implement build_proxy_cost_fn(generators, batteries, buses, demand_ref)
...
[ ] W2-B  Implement build_bqm(generators, batteries, buses, demand_ref, lambda1, lambda2)
          Constructs dimod.BinaryQuadraticModel.
          Subtasks (sequential within this task):
            [ ] linear terms (c_min, P_budget, P_infeas linear)
            [ ] quadratic u-u terms
            [ ] quadratic s-s terms
            [ ] cross u-s terms
...
[ ] W3-A  Implement Qiskit VQA path
          Depends on: W2-A (proxy_cost_fn)
          Subtasks (sequential):
            [ ] build_butterfly_ansatz(n_qubits, n_layers) — adapt from uc_10gen_benchmark.py:170
            [ ] COBYLA objective with 512-shot StatevectorSampler, lazy ⟨Q⟩ evaluation
            [ ] run_vqa_qiskit() — 300-iter COBYLA, then 5000-shot final sample,
                filter Σs_i==B, deduplicate, return top n_candidates

[ ] W3-B  Implement D-Wave SA path
          Depends on: W2-B (bqm)
          Subtasks (sequential):
            [ ] run_dwave_sa() — SimulatedAnnealingSampler, num_reads=max(2000, 10×n_candidates)
            [ ] filter Σs_i==B, sort by energy, return top n_candidates

[ ] W3-C  Implement classical second-stage evaluation
          Depends on: W1-A (QuantumSitingResult)
          Subtasks (sequential):
            [ ] candidate decoder: s bits → bat_locs, u bits → commitment list
            [ ] ED wrapper: zero p_min/p_max for off generators, call run_ed()
            [ ] UC wrapper: call run_uc() with bat_locs as-is
            [ ] evaluate_candidates() — iterate C*, dispatch to ED or UC, collect results

---

## Wave 4 — Start after W3-A, W3-B, W3-C all done

[ ] W4-A  Implement run_quantum_siting(grid, assets, T, backend, n_candidates, second_stage)
...
Run all checks independently, flag failures:

[ ] V1  D-Wave path end-to-end: n_candidates=5, second_stage=ED, T=4h, default assets
        Pass: 5 candidates evaluated, best printed with battery locations and cost
