...
Source: email from the PLEXOS side, 2026-07-22/23. Up to 4 of 14 candidate BESS units allowed to build, no line losses, no contingencies.

Note: the PLEXOS side switched from the sequential solver (which was hanging) to a statistical/sampling optimization approach for this run. He expects this changes only the precision of interval values, not the nature of the results, and the no-build case has already been benchmarked against this.

No objective/cost value is provided in any of the files for these solutions, so relative quality between sol=0 and sol=1-4 cannot be judged from the data alone. Flagged to the PLEXOS side as an open question.
...
## Open questions for the PLEXOS side
...
- Confirm whether sol=0 is intended as the primary/optimal solution and sol=1-4 are the "suboptimal" alternates he mentioned.
...
Bottom line: to compare solutions apples-to-apples, ask the PLEXOS side for either (a) the objective value per solution directly, or (b) the hourly dispatch data for sol=1-4 as well.

## Cross-check against our own repo's siting solver

Ran our repo's Battery Siting (MIP) dashboard (Phase 2 - /Git_Repo/square-peg-quantum-challenge, `ieee14` use case, `4batt_dcbus4.py` assets — 4 identical 50 MW / 200 MWh batteries, 200 MW flat datacenter at bus 4, no line losses, T=24h) against the same 14-candidate-bus siting problem, using our own objective (production + startup cost). The Benders search evaluated 144 distinct placements before stalling; only one of the PLEXOS side's 5 solutions (sol=3) showed up in that 144, all four bus-4-containing ones were absent.

That initially looked like our model disfavoring bus 4 (the datacenter bus). It doesn't: directly re-solving all 5 of the PLEXOS side's exact placements with `run_uc` (bypassing Benders' partial search) gives:

| the PLEXOS side's sol | Buses | Cost in our model | Congested hours |
|---|---|---|---|
...
All five of the PLEXOS side's solutions, and our own Benders incumbent, tie exactly at $193,229 in our model. This is a genuinely flat/degenerate objective surface in this scenario — many different 4-bus placements cost the same even though they congest different lines for a different number of hours — so the earlier "bus 4 is worse" read was an artifact of Benders' no-good-cut search order (it stalled at 144 evaluated placements before it happened to reach the bus-4 ties), not a real cost difference. Good news for the comparison: our model and PLEXOS agree on cost for every one of the PLEXOS side's 5 reported solutions, not just sol=3.

Open item: since so many placements tie in our model, ordering alone (rank 1 vs rank 12 vs "not found") isn't a meaningful way to validate against PLEXOS here, cost is. the PLEXOS side's PLEXOS objective values (still not provided, see above) would tell us whether PLEXOS shows the same degeneracy or actually discriminates between these placements.
